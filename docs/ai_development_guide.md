...
### 组件重构原则
1. **代码过长即拆分**：当文件超过500行，考虑进行组件拆分
2. **按功能边界拆分**：将不同功能的UI和逻辑拆分为独立组件
3. **提取通用元素**：将重复使用的UI元素提取为可复用组件
4. **独立对话框组件**：将复杂的对话框提取为单独的组件文件

示例：设置页面重构
- 从单一的`settings_screen.dart`文件（1300+行）
- 拆分为`settings_item.dart`、`api_config_dialog.dart`、`add_api_dialog.dart`等多个文件
- 主文件大小缩减为约300行，便于维护和理解
